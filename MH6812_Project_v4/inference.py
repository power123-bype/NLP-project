...
def check_and_fix_brightness(image_path, instruction, predicted_b, predicted_t):
    """
    Heuristic rule to fix brightness and color temperature for dark images.
    """
    try:
        # 1. Check Image Brightness
        img = Image.open(image_path).convert('L')
        stat = ImageStat.Stat(img)
        avg_brightness = stat.mean[0] / 255.0 # 0.0 - 1.0
        
        # 2. Check Instruction Intent
        text_lower = instruction.lower()
        bright_keywords = ["bright", "exposure", "daylight", "sun", "reveal", "white", "clear"]
        neutral_keywords = ["cool", "neutral", "no red", "no orange", "white", "natural", "clean"]
        
        wants_bright = any(k in text_lower for k in bright_keywords)
        wants_neutral = any(k in text_lower for k in neutral_keywords)
        
        final_b = predicted_b
        final_t = predicted_t
        
        # 3. Apply Brightness Fix
        if avg_brightness < 0.4 and wants_bright and predicted_b < 0.2:
            print(f"  [Logic Fix] Detected dark image (avg={avg_brightness:.2f}) with brightening instruction.")
            print(f"  [Logic Fix] Model predicted brightness {predicted_b:.4f}, which is too low.")
            final_b = max(predicted_b + 0.5, 0.4)
            print(f"  [Logic Fix] Forcing brightness adjustment to: {final_b:.4f}")
            
        # 4. Apply Temperature Fix (Correcting Yellow/Red Tint)
        # If user wants neutral/cool tone, but model predicts very warm (> 0.3)
        if wants_neutral and predicted_t > 0.3:
            print(f"  [Logic Fix] Detected neutral/cool instruction but model predicted warm tint ({predicted_t:.4f}).")
            # Force it to be slightly cool or neutral (e.g., -0.1 to 0.0)
            final_t = 0.0 
            print(f"  [Logic Fix] Forcing temperature adjustment to: {final_t:.4f}")
            
        return final_b, final_t
        
    except Exception as e:
        print(f"Warning: Could not apply logic fix: {e}")
        return predicted_b, predicted_t
...
def predict(image_path, instruction, model_path='best_model.pth', output_image='output_edited.jpg'):
    ...
    # --- Apply Logic Fix for Brightness and Temperature ---
    b_val, t_val = check_and_fix_brightness(image_path, instruction, b_val, t_val)
    # ----------------------------------------------------
